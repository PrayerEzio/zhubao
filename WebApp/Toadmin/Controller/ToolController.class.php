		...
		if(IS_POST)
		{
           ...
           //商品信息
			...
			if(is_array($gd_list) && !empty($gd_list))
			{
				...
				{
					$gd_list_sitemap .= "<url>\r\n"."<loc>".C('SiteUrl').'/Goods/detail/id/'.$gd['goods_id'].".html</loc>\r\n"."<priority>0.7</priority>\r\n<lastmod>".date('Y-m-d',$gd['add_time'])."</lastmod>\r\n<changefreq>daily</changefreq>\r\n</url>\r\n";		
				}	
			}  
					   		
			
            $sitemap = $sitemap_head.$art_class_sitemap.$art_list_sitemap.$gd_class_sitemap.$gd_list_sitemap.$sitemap_foot;
