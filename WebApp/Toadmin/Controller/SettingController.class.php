...
class SettingController extends GlobalController {
	...
	public function base_information()
	{
		...
		{
			$data = array();
			if(!empty($_FILES['site_logo']['size']))
			{
			...
			if(!empty($_FILES['member_logo']['size']))
			{
			...
			if(!empty($_FILES['seller_logo']['size']))
			{
			...
			if(!empty($_FILES['wsc_qrc']['size']))
			{
				$param = array('savePath'=>'common/','subName'=>'','files'=>$_FILES['wsc_qrc'],'saveName'=>'wsc_qrc','saveExt'=>'');
				$up_return = upload_one($param);
				if($up_return == 'error')
				{
					$this->error('图片上传失败');
					...
					$data['wsc_qrc'] = $up_return;
				}
			}
			if(!empty($_FILES['jd_qrc']['size']))
			{
				$param = array('savePath'=>'common/','subName'=>'','files'=>$_FILES['jd_qrc'],'saveName'=>'jd_qrc','saveExt'=>'');
				$up_return = upload_one($param);
				if($up_return == 'error')
				{
					$this->error('图片上传失败');
					...
					$data['jd_qrc'] = $up_return;
				}
			}
			if(!empty($_FILES['tb_qrc']['size']))
			{
				$param = array('savePath'=>'common/','subName'=>'','files'=>$_FILES['tb_qrc'],'saveName'=>'tb_qrc','saveExt'=>'');
				$up_return = upload_one($param);
				if($up_return == 'error')
				{
					$this->error('图片上传失败');
					...
					$data['tb_qrc'] = $up_return;
				}
			}
			if(!empty($_FILES['weixin_qrcode']['size']))
			{
	...
	public function friendlink()
	{
		...
		if (IS_POST) {
			$id = intval($_POST['id']);
			$res = $this->linkMod->where(array('id'=>$id))->delete();
			if ($res) {
	...
	public function curdLink(){
		...
		if (IS_AJAX) {
			...
			if ($res) {
				$this->success('操作成功',U('friendlink'));
			}else {
				$this->error('操作失败');
			}
		}elseif (IS_GET){
